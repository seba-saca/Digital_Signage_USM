...
if [ ! -f "$FILES_LIST" ]; then
    ...
    echo "El archivo $FILES_LIST existe." 
fi
...
if [ $? -ne 0 ]; then
    ...
    echo "El directorio de destino $DEST_DIR está listo en el dispositivo remoto."
fi
...
scp "$PATH_CONTROL_DEVICE" "$DEST_USER@$DEST_HOST:$PATH_DESTINO_SINCRO"
if [ $? -ne 0 ]; then
