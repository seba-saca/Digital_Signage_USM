...
#Parametros
admin_name=$1
parametro_plantilla=Plantilla_4
name_output=$2
...
filtro_base+="[0:v]scale=1920:1080[fondo]; \
    [1:v]scale=iw*0.59:ih*0.57[scale_main_video]; \
    [fondo][scale_main_video]overlay=x=630:y=280[xy_video_main];\
...
do
    filtro_completo+="drawtext=textfile='${archivo_txt}':x=145:y=270:fontsize=40:fontcolor=white:fontfile=path/to/font.ttf:shadowcolor=gray@0.5:shadowx=2:shadowy=2:enable='between(t,${inicio},${fin})',"
    inicio=$((inicio + 5))
    fin=$((fin + 5))
done < "$title_grande"
