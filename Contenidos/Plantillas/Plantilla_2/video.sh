...
filtro_base+="[0:v]scale=1920:1080[fondo]; \
    ...
    [fondo][scale_main_video]overlay=x=100:y=318[xy_video_main];\
...
do
    filtro_completo+="drawtext=textfile='${archivo_txt}':x=100:y=800:fontsize=48:fontcolor=white:fontfile=path/to/font.ttf:shadowcolor=gray@0.5:shadowx=2:shadowy=2:enable='between(t,${inicio},${fin})',"
    inicio=$((inicio + 5))
    fin=$((fin + 5))
done < "$title_chico"
...
while IFS= read -r archivo_txt
do
    filtro_completo+="drawtext=textfile='${archivo_txt}':x=1030:y=320:fontsize=40:fontcolor=white:fontfile=path/to/font.ttf:shadowcolor=gray@0.5:shadowx=2:shadowy=2:enable='between(t,${inicio},${fin})',"
    inicio=$((inicio + 5))
    fin=$((fin + 5))
done < "$title_grande"
