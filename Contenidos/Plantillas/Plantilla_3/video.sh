...
#Parametros
admin_name=$1
parametro_plantilla=Plantilla_3
name_output=$2
...
title_chico="$path_general/Titular_Pequeno.txt"
...
filtro_base+="[0:v]scale=1920:1080[fondo]; \
    ...
    [fondo][scale_main_video]overlay=x=560:y=280[xy_video_main];\
...
do
    filtro_completo+="drawtext=textfile='${archivo_txt}':x=150:y=800:fontsize=48:fontcolor=white:fontfile=path/to/font.ttf:shadowcolor=gray@0.5:shadowx=2:shadowy=2:enable='between(t,${inicio},${fin})',"
    inicio=$((inicio + 5))
    fin=$((fin + 5))
done < "$title_chico"
...
while IFS= read -r archivo_txt
do
    filtro_completo+="drawtext=textfile='${archivo_txt}':x=1380:y=300:fontsize=40:fontcolor=white:fontfile=path/to/font.ttf:shadowcolor=gray@0.5:shadowx=2:shadowy=2:enable='between(t,${inicio},${fin})',"
    inicio=$((inicio + 5))
    fin=$((fin + 5))
done < "$title_grande"
...
while IFS= read -r archivo_txt
do
    filtro_completo+="drawtext=textfile='${archivo_txt}':x=150:y=300:fontsize=40:fontcolor=white:fontfile=path/to/font.ttf:shadowcolor=gray@0.5:shadowx=2:shadowy=2:enable='between(t,${inicio},${fin})',"
    inicio=$((inicio + 5))
    fin=$((fin + 5))
done < "$title_grande_2"
