...
case $OPTION in
    ...
    10)
        ...
        # Verificar la conexión SSH sin contraseña antes de intentar copiar el archivo
        ...
        if ssh "$remote_host" "cat '$remote_file'" > "$local_file" 2>/dev/null; then
            echo "Archivo copiado exitosamente."
        else
            echo "No se pudo encontrar el archivo remoto. Se creó un archivo local con un descargo de responsabilidad."
            echo "No se pudo establecer conexión con el archivo remoto en $remote_host." > "$local_file"
        fi
        ;;
    *)
